...
2. Download the file matching `minimax-agent-<version>-1-x86_64.pkg.tar.zst`:
   ```bash
   wget https://github.com/unn-Known1/minimax-agent-linux/releases/download/v3.0.67/minimax-agent-3.0.67-1-x86_64.pkg.tar.zst
   ```
3. Install with pacman:
   ```bash
   sudo pacman -U minimax-agent-3.0.67-1-x86_64.pkg.tar.zst
   ```
...
sudo dnf install ./minimax-agent-3.0.67.x86_64.rpm
```
